...
def sequence_loss(flow_preds, flow_gt, gamma=0.8, max_flow=MAX_FLOW):
    ...
    # exlude extremely large diplacements
    ...
    metrics = {
        ...
        'flow_mean' : flow_preds[-1].abs().mean(),
    }
...
def gradient_loss(pred, gt):
    ...
    dx_p, dy_p = get_gradient(pred)
    dx_g, dy_g = get_gradient(gt)
    return F.l1_loss(dx_p, dx_g) + F.l1_loss(dy_p, dy_g)

def compute_loss(flow, flow_gt, gamma=0.8):
    n_predictions = len(flow)
    flow_loss = 0.0
    for i in range(n_predictions):
        i_weight = gamma**(n_predictions - i - 1)
        i_loss = (flow[i] - flow_gt).abs()
        i_grad_loss = gradient_loss(flow[i], flow_gt)
        flow_loss += i_weight * (i_loss.mean() + 0.3 * i_grad_loss.mean())
    # flow_loss += 0.1 * gradient_loss(flow[-1], flow_gt)
    loss_deriv_dict = {}
    loss_deriv_dict['flow_mean'] = flow[-1].abs().mean()
...
def get_class_metric(metric_dict, len_dict):
    ...
    for k, v in metric_dict.items():
        if k in CLASS_A:
            ...
            d_metric_sum += v * len_dict[k]
            d_len_sum += len_dict[k]
    
    return a_metric_sum/a_len_sum, b_metric_sum/b_len_sum, c_metric_sum/c_len_sum, d_metric_sum/d_len_sum
...
def calculate_rmse(pred_flow, gt_flow):
    pred_flow = pred_flow.squeeze(dim=0).permute([1,2,0]).cpu().numpy()
    gt_flow = gt_flow.squeeze(dim=0).permute([1,2,0]).cpu().numpy()
    
    error = pred_flow - gt_flow
    squared_error = np.square(error)
    squared_error_sum = np.sum(squared_error, axis=-1)
    mse = np.mean(squared_error_sum)
    rmse = np.sqrt(mse)
...
def vis_flow(forw_flow, save_path, suffix='forw_flow'):
    # forward flow
    for ii in range(forw_flow.shape[0]):
        flow = forw_flow[ii, :]
        # print(flow.size())
