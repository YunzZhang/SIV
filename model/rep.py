...
class DetailPreservingDownscaleing(nn.Module):
    def __init__(self, in_planes, planes, kernel_size=(5,3,3), padding=(0,1,1), stride=(1,2,2)):
        super(DetailPreservingDownscaleing, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        self.branch1 = nn.Conv3d(in_planes, planes, kernel_size, padding=padding, stride=stride)
        self.branch2 = nn.Sequential(
            nn.MaxPool3d(kernel_size=(1,2,2), stride=(1,2,2)), 
            nn.Conv3d(in_planes, planes, kernel_size=(5, 1, 1), stride=(stride[0], 1, 1))
        )
    def forward(self, x):
        y = x
        out1 = self.branch1(x)
        ...
        if out1.shape != out2.shape:
            raise ValueError(f'Shape mismatch: out1 shape {out1.shape} != out2 shape {out2.shape}')
        
        return self.relu(out1 + out2)

class DPHT(nn.Module):
    def __init__(self, disable_RFD=False):
        super(DPHT, self).__init__()
        ...
        if disable_RFD:
            self.conv3d_to_res_1_2 = nn.Sequential(
                nn.Conv3d(1, 16, kernel_size=(5,3,3), padding=(0,1,1), stride=(1,2,2)),  # T: (21 - 5 + 1) = 17 
            ...
            self.conv3d_to_res_1_4 = nn.Sequential(
                nn.Conv3d(16, 32, kernel_size=(5,3,3), padding=(0,1,1), stride=(2,2,2)),  # T: ((17 - 5 + 1) + 1) / 2 = 7
            ...
            self.conv3d_to_res_1_8 = nn.Sequential(
                nn.Conv3d(32, 48, kernel_size=(5,3,3), padding=(0,1,1), stride=(2,2,2)),  # T: ((7 - 5 + 1) + 1) / 2 = 2
                # nn.InstanceNorm3d(32),
                nn.ReLU(),
            )
        else:
            self.conv3d_to_res_1_2 = DetailPreservingDownscaleing(1, 16, kernel_size=(5,3,3), padding=(0,1,1), stride=(1,2,2))
            self.conv3d_to_res_1_4 = DetailPreservingDownscaleing(16, 32, kernel_size=(5,3,3), padding=(0,1,1), stride=(2,2,2))
            self.conv3d_to_res_1_8 = DetailPreservingDownscaleing(32, 48, kernel_size=(5,3,3), padding=(0,1,1), stride=(2,2,2))
        ...
        self.conv_fuse_1_2 = nn.Conv2d(16*17, 32, kernel_size=3, padding=1)
        ...
        self.conv_fuse_1_4 = nn.Conv2d(32*7, 48, kernel_size=3, padding=1)
        ...
        self.conv_fuse_1_8 = nn.Conv2d(48*2, 64, kernel_size=3, padding=1)
